...
class TablePumpModel(QtCore.QAbstractTableModel):
    ...
    def createEditor(self, parent, option, index):
        editor = super(TablePumpModel, self).createEditor(parent, option, index)
        if isinstance(editor, QtWidgets.QLineEdit):
            editor.setStyleSheet("border: none; padding: 2px;")
        return editor
...
# Create a customized QHeaderView that renders HTML in the headers
class HtmlHeaderView(QtWidgets.QHeaderView):
    def __init__(self, orientation, parent=None):
        super(HtmlHeaderView, self).__init__(orientation, parent)
    ...
    def paintSection(self, painter, rect, logicalIndex):
        ...
        if self.orientation() == QtCore.Qt.Horizontal:
            ...
            style = self.style()
            style.drawControl(QtWidgets.QStyle.CE_Header, option, painter)
            ...
            doc.setHtml(
                self.model().headerData(
                    logicalIndex, self.orientation(), QtCore.Qt.DisplayRole
                )
            )
            ...
            textLayout.draw(painter, ctx)

            painter.restore()
        else:
            super(HtmlHeaderView, self).paintSection(painter, rect, logicalIndex)
