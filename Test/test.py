...
class HistoryDialogs(unittest.TestCase):
    ...
    def test_Delete(self):
        ...
        assert hist.loc.value == "Uninstalled"
...
class PMDialogs(unittest.TestCase):
    def setUp(self):
        ...
        self.win.datas.types.append(Type(1, "type"))
        self.win.datas.freqs.append(Frequency(1, 1))
        self.win.datas.freqs.append(Frequency(2, 6))
        self.win.datas.freqs.append(Frequency(3, 12))
        self.win.datas.locs.append(Location(1, "Uninstalled"))
        self.win.datas.locs.append(Location(2, "location 1"))
        self.win.datas.locs.append(Location(3, "location 2"))
    ...
    def test_add_with_associated_pm(self):
        self.pmwin.close()
        hist = LocHistory(
            1, 2, self.win.datas.loc(3), Utility.datetime_to_str(datetime.today())
        )
        self.win.datas.history.append(hist)
        self.win.datas.pumps.append(
            Pump(
        ...
        self.pmwin = PMWindow(self.win.datas.pumps[1], parent=self.pumpwin)
        self.pmwin.show()
        self.pmwin.addbutton.click()
        self.pmwin.pmdate.setDate(QDate(datetime.today() + relativedelta(days=1)))
        self.pmwin.freqcombo.setCurrentIndex(2)
        self.pmwin.comtext.setPlainText("com")

        self.pmwin.validatebutton.click()

        pms = [pm for pm in self.win.datas.pm if pm.pump_id == 2]

        assert len(pms) == 3
        associated_pms = [pm for pm in pms if (pm.freq.id == 2 or pm.freq.id == 1)]
        for pm in associated_pms:
            assert "Generated by" in pm.com

        with open("{0}pm.json".format(self.data_dir), "r") as openfile:
            file_content = json.load(openfile)

        selected_pms = [pm for pm in file_content if int(pm["pump_id"]) == 2]
        pms = [
            PM(
                pm["id"],
                pm["pump_id"],
