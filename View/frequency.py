...
class FreqWindow(BasicDialog):
    ...
    def deleteclicked(self):
        if len(self.list.selectionModel().selection().indexes()) > 0:
            ...
            freqtodelete = next(freq for freq in self.datas.freqs if freq.id == freq_id)
            freq_in_rule = freqtodelete.exist([Frequency(rule.freq.id, rule.freq.value) for rule in self.datas.rules])
            if freq_in_rule is None:
                ...
                if freq_in_struc is None:
                    ...
                    if freq_in_pm is None:
                        ...
                        messageBox("Frequency used in pm", "You can't delete a frequency used in a pm. Please manage the pm first !")
                else:
                    messageBox("Frequency used in structure", "You can't delete a frequency used in the structure. Please update the structure first !")
            else:
                messageBox("Frequency used in rules", "You can't delete a frequency used in a rule. Please update the rules first !")
        else:    
        ...
        if self.text.text() != "":      
            if self.text.text().isnumeric():
                ...
                if temp_frequency is None or temp_frequency == self.selectedItem:                         
                    ...
                    messageBox("New frequency error", "New Freqcuency can't have the same value that existing frequency !")
            else:
    ...
    def addFreq(self):
        if len(self.datas.freqs) > 0:
            newid = max([int(freq.id) for freq in self.datas.freqs]) + 1
        else:
            newid = 1
        frequency = self.text.text()
        freq = Frequency(newid, frequency)
        self.model.addItem(freq)
        self.selectedItem = freq
        self.datas.save_frequency()
    ...
    def modifyFreq(self):
        ...
        for i, freq in enumerate(self.datas.freqs):
            if freq.id == selected_id:
                self.datas.freqs[i].value = self.text.text()
                self.model.updateItem(i, self.datas.freqs[i])  
                self.selectedItem = self.datas.freqs[i]
                self.datas.save_frequency()
                break