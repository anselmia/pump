...
class TypeWindow(BasicDialog):
    ...
    def deleteclicked(self):
        if len(self.list.selectionModel().selection().indexes()) > 0:
            ...
            type_in_struct = temp_type.exist([struct.type for struct in self.datas.struct])
            if type_in_struct is None:
                type_in_pumps = temp_type.exist([pump.type for pump in self.datas.pumps])
                if type_in_pumps is None:
                    self.model.removeItem(del_index.row())
                    self.datas.save_type()
                    self.selectedItem = None
                else:
                    messageBox("Type used in a pump", "You can't delete a type used in a pump. Please manage the pump first !")
            else:
                messageBox("Type used in structure", "You can't delete a type used in the structure. Please update the structure first !")
        else:
    ...
    def validateclicked(self):
        if self.text.text() != "":   
            ...
            if temp_type is None or temp_type == self.selectedItem:                    
                ...
                messageBox("New Type error", "New pump Type can't have the same value that existing pump type !")     
        else:
            messageBox("Missing Information", "Please review the missing information(s)")    
    
    def addType(self):
        if len(self.datas.types) > 0:
            newid = max([int(type.id) for type in self.datas.types]) + 1
        else:
            newid = 1
    ...
    def modifyType(self):
        ...
        for i, type in enumerate(self.datas.types):
            if type.id == selected_id:
                ...
                self.datas.save_type()
                self.selectedItem = self.datas.types[i]
                break